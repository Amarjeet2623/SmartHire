...
const mockResumeData: ResumeInputData = {
  personalInfo: {
    name: "Rahul Gogra",
    email: "rahul.gogra@example.com",
    phone: "7976608292",
    linkedin: "linkedin.com/in/rahulgogra",
    portfolio: "protfolio.rahulgogra.com",
    address: "123 Tech Street, Jaipur, Rajasthan, India",
  },
  summary: "Highly motivated and results-oriented Software Engineer experience in developing and deploying scalable web applications. Proficient in full-stack development with expertise in React, Node.js, and cloud technologies. Seeking to leverage strong problem-solving skills and a passion for innovation in a challenging role at a forward-thinking company.",
  ...
  experience: [
    ...
    },
    {
      ...
      company: "Blueplanet Infosolutions pvt. ltd.",
  ...
  education: [
    {
      ...
      endDate: "present",
      details: ["Learned about Web development, C++, Data Structures and algorithms", "GPA: 7.12/10"],
    },
  ],
  skills: [
    { category: "Programming Languages", items: ["JavaScript (ES6+)", "Python", "C++", "TypeScript", "SQL"] },
    { category: "Frameworks/Libraries", items: ["React", "Node.js", "Express.js", "Next.js"] },
    { category: "Databases", items: ["MongoDB", "PostgreSQL", "MySQL", "Redis"] },
    { category: "Tools/Platforms", items: ["Git", "Docker", "AWS", "Kubernetes"] },
...
const ResumeBuilder = () => {
  ...
  const handleInputChange = (section: keyof ResumeInputData, field: string, value: unknown, index?: number) => {
    setUserInfo(prev => {
      ...
      if (index !== undefined && Array.isArray(newState[section])) {
        ...
        (newState[section] as unknown)[field] = value;
      } else {
        (newState as unknown)[field] = value;
      }
      return newState;
    });
  };
  ...
  const generateResumeContent = async () => {
    ...
    try {
      ...
      if (response.status === 201 && response.data.generatedText && response.data.generatedResumeId) {
        const text = response.data.generatedText.replace(/```json|```/g, '').trim();
        // Parse JSON
        const resumeData: ResumeInputData = JSON.parse(text);
        setGeneratedText(resumeData); // store JSON object
  ...
  //     // Request PDF as a blob so the browser knows it's binary data
  //     const response = await apiClient.get(`/api/builder/download/${generatedResumeId}`, {
  //       responseType: 'blob', // ensures we get the raw PDF
  //     });
  ...
  //     link.href = url;
  //     link.setAttribute('download', 'resume.pdf'); // sets the filename
                ...
                <ScrollArea className="h-[60vh] w-full">
                  {generatedText ? (
                    <div id="resume-container">
                      <ResumeTemplate resume={generatedText} />
                    </div>
                  ) : (
                    <p className="text-gray-500 text-center mt-8">No resume generated yet</p>
                  )}
                </ScrollArea>
              </CardContent>
            </Card>
